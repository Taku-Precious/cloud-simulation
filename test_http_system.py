"""
Simple HTTP-based System Test Suite
Tests all components via REST API only - no module imports
"""
import requests
import json
import time
...
try:
    ...
    if response.status_code == 201:
        ...
        results.add_fail("Register User", f"Status {response.status_code}")
except Exception as e:
...
try:
    ...
    if response.status_code == 200:
        ...
        if session_id:
            results.add_pass("Login Request", f"OTP sent, session_id received")
        else:
            results.add_fail("Login Request", "No session_id in response")
    else:
        results.add_fail("Login Request", f"Status {response.status_code}: {response.text}")
except Exception as e:
    results.add_fail("Login Request", str(e))

# Get OTP from database for testing
if session_id:
    print(f"\nTest 2.2b: Retrieving OTP from database...")
    try:
        ...
        results.add_fail("OTP Retrieval", str(e))
...
try:
    if session_id and otp_code:
        ...
        if response.status_code == 200:
            ...
            results.add_fail("OTP Verification", f"Status {response.status_code}")
    else:
...
if not auth_token:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if file_id:
                results.add_pass("Upload File", f"File uploaded, ID: {file_id[:20]}...")
            else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            results.add_pass("List Files", f"Found {len(files_list)} file(s)")
        else:
        ...
        try:
            ...
            if response.status_code == 200:
                if response.content == test_file_content:
                    results.add_pass("Download File", "Content verified")
                else:
                    results.add_fail("Download File", "Downloaded content doesn't match")
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 404:
                    ...
                    results.add_fail("Verify Deletion", f"File still accessible (status {response.status_code})")
            else:
